...
function scrape_race_runners($first_start)
{
    ...
    LogMessage("Number of races to scrape this minute - ".count($listOfRace));

    // print out array later on when we need the info on the page
    foreach($listOfRace as $race){
		LogMessage("Starting Scraping for ".$race["tatts_track_name"].", Scrape Time : ".$race["next_scrape_time"].", Slab : ".$race["race_slab_no"]);
        scrape_race_runner_detail($race);
		//LogMessage("Completed for Scrape Time : ".$race["next_scrape_time"]);
    }
...
function get_mins_to_next_scrape($current_time, $raceTime)
{
    ...
    if (($minToNextScrape<=21)&&($minToRaceStart>21)){
        ...
        $race_slab = 8;
        //$minToNextScrape = 20;
    }elseif (($minToNextScrape<=20)&&($minToRaceStart>20)){
        $race_slab = 6;
        $minToNextScrape = 20;
    }elseif (($minToNextScrape>=20)&&($minToNextScrape<21)){
        $race_slab =6;
        //$minToNextScrape = 15;
    }elseif (($minToNextScrape<=19)&&($minToRaceStart>19)){
        $race_slab = 5;
        $minToNextScrape = 19;
    }elseif (($minToNextScrape>=19)&&($minToNextScrape<20)){
        $race_slab = 5;
        //$minToNextScrape = 15;
    }elseif (($minToNextScrape<=11)&&($minToRaceStart>11)){
        $race_slab = 11;
        $minToNextScrape = 11;
    }elseif (($minToNextScrape>=11)&&($minToNextScrape<19)){
        $race_slab =11;
        //$minToNextScrape = 15;
    }elseif (($minToNextScrape<=9)&&($minToRaceStart>9)){
        $race_slab = 19;
        $minToNextScrape = 9;
    }elseif (($minToNextScrape>=9)&&($minToNextScrape<11)){
        $race_slab = 19;
        //$minToNextScrape = 15;
    }elseif (($minToNextScrape<=7)&&($minToRaceStart>7)){
        $race_slab = 17;
        $minToNextScrape = 7;
    }elseif (($minToNextScrape>=7)&&($minToNextScrape<9)){
        $race_slab = 17;
        //$minToNextScrape = 15;
    }elseif (($minToNextScrape<=5)&&($minToRaceStart>5)){
        $race_slab = 4;
        $minToNextScrape = 5;
    }elseif (($minToNextScrape>=5)&&($minToNextScrape<7)){
        $race_slab = 4;
        //$minToNextScrape = 15;
        ...
        $race_slab = 2;
        //$minToNextScrape = 15;
    }elseif (($minToNextScrape>=1)&&($minToNextScrape<2)){
        $race_slab = 1;
        //$minToNextScrape = 15;
...
function update_runner_data($runner_details, $runner_id, $raceSlab)
{
    ...
    $slabUpdate="";
     ...
     if ($raceSlab==7){
        $slabUpdate = ", tatts_runr_win_21 = ".$runner_details->runr_fix_win;
    }elseif($raceSlab==6){
        $slabUpdate = ", tatts_runr_win_20 = ".$runner_details->runr_fix_win;
    }elseif($raceSlab==5){
        $slabUpdate = ", tatts_runr_win_19 = ".$runner_details->runr_fix_win;
    }elseif($raceSlab==4){
        $slabUpdate = ", tatts_runr_win_4 = ".$runner_details->runr_fix_win;
    }elseif($raceSlab==3){
        $slabUpdate = ", tatts_runr_win_3 = ".$runner_details->runr_fix_win;
    }elseif($raceSlab==2){
        $slabUpdate = ", tatts_runr_win_2 = ".$runner_details->runr_fix_win;
    }elseif($raceSlab==1){
        $slabUpdate = ", tatts_runr_win_1 = ".$runner_details->runr_fix_win;
    }elseif($raceSlab==1){
...
function update_runner_sb_data($sb_win_value, $runner_id, $raceSlab)
{
    ...
    if($db2->affected_rows > 0){
        //LogMessage("Number new  inserted ". $db2->affected_rows);
    }
}
...
function update_runner_bf_data($bf_win_value_b, $bf_win_value_L, $runner_id, $raceSlab)
{
    ...
    //LogMessage("For Runner ID :".$runner_id."--Slab No : ".$raceSlab."--");
     ...
     if ($raceSlab==7){
        $slabUpdate = ", bf_runr_win_21_b = ".$bf_win_value_b.", bf_runr_win_21_L = ".$bf_win_value_L;
    }elseif($raceSlab==6){
        $slabUpdate = ", bf_runr_win_20_b = ".$bf_win_value_b.", bf_runr_win_20_L = ".$bf_win_value_L;
    }elseif($raceSlab==5){
        $slabUpdate = ", bf_runr_win_19_b = ".$bf_win_value_b.", bf_runr_win_19_L = ".$bf_win_value_L;
    }elseif($raceSlab==11){
        $slabUpdate = ", bf_runr_win_11_b = ".$bf_win_value_b.", bf_runr_win_11_L = ".$bf_win_value_L;
    }elseif($raceSlab==19){
        $slabUpdate = ", bf_runr_win_9_b = ".$bf_win_value_b.", bf_runr_win_9_L = ".$bf_win_value_L;
    }elseif($raceSlab==17){
        $slabUpdate = ", bf_runr_win_7_b = ".$bf_win_value_b.", bf_runr_win_7_L = ".$bf_win_value_L;
    }elseif($raceSlab==4){
        $slabUpdate = ", bf_runr_win_4_b = ".$bf_win_value_b.", bf_runr_win_4_L = ".$bf_win_value_L;
    }elseif($raceSlab==3){
        $slabUpdate = ", bf_runr_win_3_b = ".$bf_win_value_b.", bf_runr_win_3_L = ".$bf_win_value_L;
    }elseif($raceSlab==2){
